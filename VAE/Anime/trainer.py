   ...
   def evaluate(self, tr_dataloader, te_dataset, mode='validation'):
      ...
      with torch.no_grad():
         for batch_idx, x in enumerate(tr_dataloader):
            ...
            n_20 = NDCG_binary_at_k_batch(recon_batch, heldout_data, 20)
            n_10 = NDCG_binary_at_k_batch(recon_batch, heldout_data, 10)
            r_50 = Recall_at_k_batch(recon_batch, heldout_data, 50)
            r_20 = Recall_at_k_batch(recon_batch, heldout_data, 20)
            r_10 = Recall_at_k_batch(recon_batch, heldout_data, 10)
            r_5 = Recall_at_k_batch(recon_batch, heldout_data, 5)
            r_1 = Recall_at_k_batch(recon_batch, heldout_data, 1)
            

            n100_list.append(n_100)
            n20_list.append(n_20)
            n10_list.append(n_10)
            r50_list.append(r_50)
            r20_list.append(r_20)
            r10_list.append(r_10);
            r5_list.append(r_5);
            r1_list.append(r_1);
      ...
      # print(f"final length n100={len(n100_list)} r20={len(r20_list)} r50={r50_list.shape} total_loss={total_loss}");

      if mode=='validation':
         
         n100 = np.mean(n100_list)
         r20 = np.mean(r20_list)
         n20 = np.mean(n20_list)
         n10 = np.mean(n10_list)
      ...
      self.epochs += 1
      return total_loss, n100_list, n20_list,n10_list,r50_list,r20_list,r10_list,r5_list,r1_list;
