...
class MovieLens_DataModule(object):
   ...
   def setup(self):   
      ...
      # Split Users
      tr_users, vd_users, te_users = split_users(unique_uid,n_heldout_users=400);
...
class Books_DataModule(object):
   ...
   def read_data(self, path):
      ...
      if path.endswith('dat'):
         return pd.read_csv(path, names=["userId", "movieId", "rating"],  sep="\s+", skiprows = 1 )
   ...
   def setup(self):   
      ...
      self.prepare_data()
      print("about to read")
      raw_data = self.read_data(path=os.path.join(self.args.data_dir, 'book_history.dat'))
...
class Anime_DataModule(object):
   ...
   def prepare_data(self):
      ...
      file_list = os.listdir(self.args.data_dir)
      dat_files = [file for file in file_list if file.endswith(".dat")]
      print(f"dat_files={set(dat_files)}");
      if len(set(dat_files))!=4:
         download_extract(self.args.data_url)
   ...
   def read_data(self, path):
      if path.endswith('csv'):
         return pd.read_csv(path, header=0)
      if path.endswith('dat'):
         df_anime =  pd.read_csv(path, names=["userId", "movieId", "rating"],  sep="\s+", skiprows = 1 )
   ...
   def setup(self):   
      ...
      self.prepare_data()
      print("about to read")
      raw_data = self.read_data(path=os.path.join(self.args.data_dir, 'anime_history.dat'))
