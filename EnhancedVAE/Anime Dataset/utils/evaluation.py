...
def evaluate_vae(args, model, train_loader, data_loader, epoch, dir, mode):
    ...
    def NDCG_binary_at_k_batch(X_pred, heldout_batch, k=100):
        ...
        idcg = torch.tensor([[(tp[:min(n, k)]).sum() if n > 0 else 1e-8] for n in (heldout_batch != 0).sum(dim=1)])

        ndcg = DCG / idcg.squeeze()

        # Handle cases where heldout_batch has no positive items
    ...
    def Recall_at_k_batch(X_pred, heldout_batch, k=100):
        ...
        X_true_binary = torch.tensor((heldout_batch > 0), dtype=torch.float).to(device)
        tmp = torch.tensor(np.logical_and(X_true_binary.cpu().numpy(), X_pred_binary), dtype=torch.float).sum(dim=1)
        #recall = tmp / np.minimum(k, X_true_binary.cpu().sum(dim=1))
        #return recall

        recall = np.zeros(batch_users)
        mask = (X_true_binary.cpu().sum(dim=1) > 0)
        recall[mask] = tmp[mask] / np.minimum(k, X_true_binary.cpu().sum(dim=1)[mask])
