...
def experiment_vae(args, train_loader, val_loader, test_loader, model, optimizer, dir, log_dir, model_name='vae'):
    ...
    plt.plot(np.arange(1, len(train_loss_history)+1), train_loss_history, label='x')
    plt.plot(np.arange(1, len(val_loss_history)+1), val_loss_history, label='y')

    # Add a legend and labels
    plt.legend()
    plt.xlabel('Index + 1')
    plt.ylabel('Value')
    plt.title('Plot of x and y against Index + 1')
